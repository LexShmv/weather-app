    ...
    <template #icon>
      ...
      >
		...
		C106.548,330.355,113.701,325.842,119.697,322.008z"
          />
          <circle cx="273.853" cy="212.766" r="25" />
        </g>
      </svg>
    </template>

    <template #name>Wind</template>

    <template #info>
      <p>
        Speed: <span class="info__data">{{ wind.speed }}</span> m/s
      </p>
      <p>
        Direction: <span class="info__data">{{ wind.deg }}</span> degrees
      </p>
      <p>
        Gust: <span class="info__data">{{ wind.gust }}</span> m/s
      </p>
    </template>
  </WidgetContainer>
</template>
